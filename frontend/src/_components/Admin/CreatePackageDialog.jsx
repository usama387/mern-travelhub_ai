...
import { useCreateNewPackageMutation } from "@/features/api/packageApi";
import { toast } from "sonner";

const CreatePackageDialog = ({ isOpen, onClose }) => {
  const [createNewPackage, { isLoading }] = useCreateNewPackageMutation();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      submitData.append('complementaryBreakfast', formData.complementaryBreakfast.toString());
      submitData.append('pickAndDrop', formData.pickAndDrop.toString());
      submitData.append('features', JSON.stringify(formData.features));
      submitData.append('image', selectedFile);

      // Call the mutation
      const result = await createNewPackage(submitData).unwrap();
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  handleInputChange("hotelType", value)
                }
                disabled={isLoading}
              >
                <SelectTrigger>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex items-center justify-between">
              <Label htmlFor="complementaryBreakfast" className="text-sm font-medium">
                Complementary Breakfast
              </Label>
              <Switch
                id="complementaryBreakfast"
                checked={formData.complementaryBreakfast}
                onCheckedChange={(checked) => handleToggleChange("complementaryBreakfast", checked)}
                disabled={isLoading}
              />
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <Switch
                ...
                onCheckedChange={(checked) => handleToggleChange("pickAndDrop", checked)}
                disabled={isLoading}
              />
            </div>
          </div>
